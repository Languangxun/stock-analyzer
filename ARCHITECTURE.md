...
| `~/ai-quant/` | 下游系统 | 接收 `--push` 推送的 `stock_<code>_<date>.json` |
...
### 4.4 版本链
`v6.1.3` → `v6.1.2`（ETF 四口径）→ `v6.1.1`（激进档对标科创50 / 主板激进重做）→ `v6.1`（标准回测 + AI/设置/搜索）
→ `v6.0`（三档生产化 + 全库数据修复）→ `v5.0` → `v4.0.x`。
