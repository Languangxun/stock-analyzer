...
- **ETF/LOF**：`refresh_etf_codes()` 拉东财四板块（`b:MK0021..MK0024`），
  写入 `stocks`（`industry='ETF'`，含规模）；**剔除货币/现金类**（名称含 货币/快线/快钱/添益/日利/现金/理财/短融，
...
- **ETF 历史回填**：`backfill_etf_history()` 走主库 hfq 路径 + `_sync_adjust`，断点续传（≥1000 根且新鲜则跳过）、6 线程、全局节流；
  实测：代码表 **1491 只** → 有 K 线 **1202** → ≥250 根 **1145**（452 只 1000+ 根），合计 **90.7 万根**；
...
- **深历史回填**：GUI 设置【全市场回填】/ CLI `--backfill`；腾讯 2×800 翻页为主源，东财兜底；501 全局暂停自愈 + 断点续传 + 6 线程；
