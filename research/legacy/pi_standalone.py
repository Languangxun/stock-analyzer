...
def load_pools_progressive(full, ctx, progress=None, batch=12):
    ...
    for key, codes in order:
        ...
        for bi, i in enumerate(range(0, len(codes), batch), 1):
            ...
            if progress:
                progress(f"后台加载{LV_LABEL[key]}样本 第{bi}/{n_batches}批 "
                         f"(共{len(codes)}只)...")
            try:
            ...
            if time.time() - t0 > 40:   # 单级超时保护
                if progress:
                    progress(f"{LV_LABEL[key]}池加载超时(40s)，"
...
class App:
    ...
    def _build_toolbar(self):
        ...
        if self.compact:
            ...
            ttk.Button(top, text="⏻", width=3,
                       command=self._power_menu).pack(side="right", padx=2)
            ...
            ttk.Button(top2, text="⛶", width=3,
                       command=self._toggle_chart_fs).pack(side="right", padx=2)
            # 状态栏独占一行：紧贴代码输入行下方，蓝字加亮不淹没
            self._status_row = tk.Frame(self.root, bg=DARK_BG)
        ...
        # 进度/状态移到代码输入行下方的独立行（加亮蓝，避免看不清）
        self.progress_var = tk.StringVar(value="")
        status_row = getattr(self, "_status_row", None) or info_row
        progress_lbl = tk.Label(
            status_row, textvariable=self.progress_var,
            fg="#63b3ff", bg=DARK_BG, anchor="w",
            font=("Microsoft YaHei", 9))
        progress_lbl.pack(side="left", fill="x", expand=True,
                          padx=(8 if status_row is info_row else 6, 0))
        ...
        self._prog_lbl = progress_lbl   # 状态栏截断：小屏长文本防溢出
        ...
        def _fit_info(_e=None):
            try:
                ...
                srow = getattr(self, "_status_row", None)
                if srow is not None and srow.winfo_width() > 20:
                    pest = max(6, int((srow.winfo_width() - 14) / 9))
                    self._prog_lbl.config(
                        text=ptext[:pest] + "…" if len(ptext) > pest
                        else ptext)
            except Exception:
                pass
        info_row.bind("<Configure>", _fit_info)
        srow2 = getattr(self, "_status_row", None)
        if srow2 is not None:
            srow2.bind("<Configure>", _fit_info)
        # 变量变化时也重新适配
    ...
    def _build_body(self):
        ...
        # 小屏常态不显示K线（报告为主）；⛶全屏进入纯K线模式
        self._fs_charts_visible = not getattr(self, "compact", False)
        if not self._fs_charts_visible:
            self.cv_main.grid_remove()
            self.cv_vol.grid_remove()
            self.cv_ind.grid_remove()

        bottom = tk.Frame(self.root, bg=DARK_BG)
        bottom.pack(fill="both", padx=8, pady=(2, 6),
                    expand=getattr(self, "compact", False))
    ...
    def _toggle_chart_fs(self):
        """⛶全屏=纯K线：隐藏工具栏/状态栏/报告/副图，只留主图。
        非全屏（小屏常态）不显示K线，以报告为主。"""
        compact = getattr(self, "compact", False)
        if getattr(self, "_fs_mode", False):
            self._fs_mode = False
            try:
                self.root.attributes("-fullscreen", False)
            except Exception:
                pass
            # 按原始顺序恢复：工具栏 → 信息/状态栏 → 报告 → 右侧栏
            for w in (self._w_top, self._w_top2, self._info_row,
                      self._w_bottom, self._w_right):
                if w is None:
                    continue
                pk = next((p for ww, p in getattr(self, "_fs_restore", [])
                           if ww is w), None)
                try:
                    if pk:
                        w.pack(**pk)
                    ...
                    pass
            if self._w_idxbar is not None and not compact:
                try:
                    self._w_idxbar.grid(row=3, column=0, sticky="ew",
                                        padx=(2, 2), pady=(4, 0))
                except Exception:
                    pass
            # 小屏常态继续隐藏K线；大屏恢复量能/指标副图
            if not compact:
                for row, cv in ((1, self.cv_vol), (2, self.cv_ind)):
                    try:
                        cv.grid(row=row, column=0, sticky="nsew", padx=(2, 2))
                    except Exception:
                        pass
            if getattr(self, "_fs_exit_btn", None):
                try:
                    self._fs_exit_btn.destroy()
                except Exception:
                    pass
                self._fs_exit_btn = None
            self.progress_var.set("已退出K线全屏")
            self._rerender()
            return
        # ---- 进入：只留主图 ----
        self._fs_mode = True
        self._fs_restore = []
        for w in (self._w_top, getattr(self, "_w_top2", None),
                  self._info_row, self._w_bottom, self._w_right):
            if w is None:
                continue
            try:
                self._fs_restore.append((w, w.pack_info()))
                w.pack_forget()
            except Exception:
                pass
        if self._w_idxbar is not None:
            try:
                self._w_idxbar.grid_remove()
            except Exception:
                pass
        for cv in (self.cv_vol, self.cv_ind):
            try:
                cv.grid_remove()
            except Exception:
                pass
        try:
            ...
            pass
        btn = tk.Label(self.root, text="✕ 退出", font=("Microsoft YaHei", 10,
                                                       "bold"),
                       bg="#333c46", fg="#ffffff", padx=8, pady=4,
                       cursor="hand2")
        btn.place(relx=1.0, rely=0.0, x=-6, y=6, anchor="ne")
        btn.lift()
        btn.bind("<Button-1>", lambda e: self._toggle_chart_fs())
        self._fs_exit_btn = btn
        self.root.bind("<Escape>", lambda e: self._toggle_chart_fs())
        self.progress_var.set("K线全屏：双击图表或点 ✕ 退出")
        self._rerender()

    def _power_menu(self, event=None):
        """电源菜单：关机/重启。"""
        m = tk.Menu(self.root, tearoff=0, font=("Microsoft YaHei", 10))
        m.add_command(label="⚠ 关机", command=self._do_power)
        m.add_command(label="↻ 重启", command=lambda: self._do_power(True))
        try:
            m.tk_popup(self.root.winfo_pointerx(), self.root.winfo_pointery())
        finally:
            m.grab_release()

    def _do_power(self, reboot=False):
        import subprocess
        cmd = ("sudo -n /sbin/reboot" if reboot
               else "sudo -n /sbin/shutdown -h now")
        if not messagebox.askyesno(
                "电源操作",
                "确认" + ("重启" if reboot else "关机") + "设备？"):
            return
        try:
            r = subprocess.run(cmd.split(), capture_output=True, text=True,
                               timeout=8)
            if r.returncode != 0:
                raise RuntimeError(r.stderr.strip() or "权限不足")
            self.progress_var.set("已下发电源指令")
        except Exception as e:
            messagebox.showerror("电源操作失败", str(e))
    ...
    def run(self):
        try:
            ...
            # 行内提示而非模态弹窗：触屏上弹窗难关闭，且不挡后续输入
            self.progress_var.set("⚠ " + str(e).replace("\n", " "))
            return
    ...
    def _drag_reset(self, _event):
        """双击图表：全屏模式退出；否则回到最新K线。"""
        if getattr(self, "_fs_mode", False):
            self._toggle_chart_fs()
            return
        if self.view_pan:
            self.view_pan = 0
    ...
    def open_settings(self):
        ...
        notes = ("【适合】区间参考(T+1/T+5已校准)｜强信号(预测>1%)｜"
                 "传统股L1+L2\n【不适合】上行概率当买卖信号｜题材股跨股参考｜"
                 "ETF按T+1交易\n方向预测无统计alpha；本工具不含仓位风控")
        ttk.Label(frm, text=notes, justify="left", foreground=AXIS_TXT,
                  font=("Microsoft YaHei", 8)).grid(
            row=0, column=0, columnspan=3, sticky="we", pady=(0, 8))

        ttk.Label(frm, text="界面主题").grid(row=1, column=0, sticky="w", pady=4)
        theme_var = tk.StringVar(value=self.settings["theme"])
        ttk.Radiobutton(frm, text="暗色", variable=theme_var,
                        value="dark").grid(row=1, column=1, sticky="w")
        ttk.Radiobutton(frm, text="亮色", variable=theme_var,
                        value="light").grid(row=1, column=2, sticky="w")

        ttk.Label(frm, text="涨跌配色").grid(row=2, column=0, sticky="w", pady=4)
        ud_var = tk.StringVar(value=self.settings["updown"])
        ttk.Radiobutton(frm, text="红涨绿跌", variable=ud_var,
                        value="red_up").grid(row=2, column=1, sticky="w")
        ttk.Radiobutton(frm, text="绿涨红跌", variable=ud_var,
                        value="green_up").grid(row=2, column=2, sticky="w")

        ttk.Label(frm, text="DeepSeek Key").grid(row=3, column=0, sticky="w",
                                                 pady=(8, 4))
        key_var = tk.StringVar(value=self.api_key)
        ent = ttk.Entry(frm, textvariable=key_var, width=42, show="*")
        ent.grid(row=3, column=1, columnspan=2, sticky="we", pady=(8, 4))
        ...
        cmb_model = ttk.Combobox(frm, textvariable=model_var, width=20,
                                 values=["deepseek-v4-pro", "deepseek-chat",
                                         "deepseek-reasoner"])
        cmb_model.grid(row=4, column=1, columnspan=2, sticky="w", pady=4)
